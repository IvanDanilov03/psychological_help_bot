...
MODEL_SAVE_PATH = 'lstm_chatbot_model'
VOCAB_SAVE_PATH = 'lstm_chatbot_vocab.pkl'
EVALUATION_SAVE_PATH = 'lstm_evaluation_results.json'
TRAINING_LOSS_PLOT_SAVE_PATH = 'lstm_training_loss.png'
...
class Seq2SeqWithAttention(tf.keras.Model):
    def __init__(self, input_vocab_size, target_vocab_size, embedding_dim=100, hidden_dim=512):
        ...
        # Attention mechanism
        self.attention = tf.keras.layers.AdditiveAttention()  # You could also use tf.keras.layers.Attention
    ...
    def call(self, inputs):
        ...
        encoder_outputs, state_h, state_c = self.encoder_lstm(encoder_embedded)  # encoder_outputs used for attention
...
# Training
def train_model(X_train, Y_train, input_vocab_size, target_vocab_size):
    model = Seq2SeqWithAttention(input_vocab_size, target_vocab_size)
    model.compile(
...
def main():
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        model = train_model(X_train, Y_train, len(input_w2i), len(target_w2i))

...
MODEL_SAVE_PATH = 'rnn_chatbot_model'
VOCAB_SAVE_PATH = 'rnn_chatbot_vocab.pkl'
EVALUATION_SAVE_PATH = 'rnn_evaluation_results.json'
TRAINING_LOSS_PLOT_SAVE_PATH = 'rnn_training_loss.png'
...
class Seq2SeqWithAttentionRNN(tf.keras.Model):
    def __init__(self, input_vocab_size, target_vocab_size, embedding_dim=100, hidden_dim=512):
        super(Seq2SeqWithAttentionRNN, self).__init__()
        ...
        # Attention mechanism
        self.attention = tf.keras.layers.AdditiveAttention()  # You can also use tf.keras.layers.Attention
    ...
    def call(self, inputs):
        ...
        encoder_outputs, state_h = self.encoder_rnn(encoder_embedded)  # encoder_outputs used for attention

        # Decoder processing
        decoder_embedded = self.decoder_embedding(decoder_input)
        decoder_outputs, _ = self.decoder_rnn(decoder_embedded, initial_state=state_h)

        # Attention mechanism
        attention_context = self.attention([decoder_outputs, encoder_outputs])  # Compute context vector
        combined_context = tf.concat([decoder_outputs, attention_context], axis=-1)  # Combine with decoder outputs
...
# Training
def train_model_with_rnn(X_train, Y_train, input_vocab_size, target_vocab_size):
    model = Seq2SeqWithAttentionRNN(input_vocab_size, target_vocab_size)
    model.compile(
...
def main():
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        model = train_model_with_rnn(X_train, Y_train, len(input_w2i), len(target_w2i))

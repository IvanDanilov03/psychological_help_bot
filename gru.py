...
MODEL_SAVE_PATH = 'gru_chatbot_model'
VOCAB_SAVE_PATH = 'gru_chatbot_vocab.pkl'
EVALUATION_SAVE_PATH = 'gru_evaluation_results.json'
TRAINING_LOSS_PLOT_SAVE_PATH = 'gru_training_loss.png'
...
class Seq2SeqWithAttentionGRU(tf.keras.Model):
    def __init__(self, input_vocab_size, target_vocab_size, embedding_dim=100, hidden_dim=512):
        super(Seq2SeqWithAttentionGRU, self).__init__()
    ...
    def call(self, inputs):
        ...
        encoder_outputs, state_h = self.encoder_gru(encoder_embedded)  # encoder_outputs used for attention

        # Decoder processing
        decoder_embedded = self.decoder_embedding(decoder_input)
        decoder_outputs, _ = self.decoder_gru(decoder_embedded, initial_state=state_h)
...
# Training
def train_model_with_gru(X_train, Y_train, input_vocab_size, target_vocab_size):
    model = Seq2SeqWithAttentionGRU(input_vocab_size, target_vocab_size)
    model.compile(
...
def main():
    ...
    if os.path.exists(MODEL_SAVE_PATH):
        ...
        model = train_model_with_gru(X_train, Y_train, len(input_w2i), len(target_w2i))
